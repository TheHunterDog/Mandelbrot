...
fn main() {
    ...
    if args.len() != 5{
      ...
      writeln!(std::io::stderr(),"Example: {} mandel.png 1000*750 - 1.20,0.35 -1,0.20", args[0]).unwrap();
      std::process::exit(1);
    }
...
fn render(pixels:&mut [u8], bounds: (usize,usize), upper_left:Complex<f64>,lower_right:Complex<f64>){
  ...
  for row in 0..bounds.1 {
    for collumn in 0..bounds.1{
      let point = pixel_to_point(bounds, (collumn,row), upper_left, lower_right);
      pixels[row*bounds.0+collumn] = match escape_time(point,255){
        None => 0,
